...
- Keep a rolling 180-day arXiv archive and a five-year publication-date Preprints.org archive directly in compact per-topic chunks under `data/`, then generate `index.html`.
...
The generated `index.html` first loads the small `data/topics.json` manifest, then fetches and caches only the chunk needed for the current page. Full-topic data is deferred until a full-text search needs it, and the complete archive is never downloaded. Preview it through a local web server:
...
- `JSON_MAX_SIZE_MIB` (default `75`) limits the combined generated shard size.
...
The existing Git history still contains older `arxiv-daily.json` snapshots. Removing that historical clone weight requires a separate, one-time history rewrite and force push; the daily workflow intentionally does not perform that destructive operation.
